...
const style = "";
const slogan = "/assets/amolosanimales.b0fe8691.png";
const googleAccess = "/assets/BtnGoogle.0e89209d.png";
const welcomePic = "/assets/CircleLogo.73fc3f84.png";
const homeHome = "/assets/homeIcon.6c72345e.png";
const likedIcon = "/assets/likePaw.56d07f8d.png";
const likeIcon = "/assets/likePawZero.a01ddddc.png";
const logo = "/assets/logopetropolissf.7f63b81c.png";
const outOut = "/assets/logOutIcon.de846bb1.png";
const profileProfile = "/assets/profileIcon.b858b9cc.png";
const home = (onNavigate2) => {
  ...
  loginButton.textContent = "Iniciar Sesi\xF3n";
  ...
  loginButton.addEventListener("click", () => onNavigate2("/login"));
  signInButton.addEventListener("click", () => onNavigate2("/register"));
  return homeSection;
};
/**
 * @license
 ...
 * limitations under the License.
 */
async function signInWithPassword$1(auth2, request) {
...
class EmailAuthCredential extends AuthCredential {
  ...
  async _getIdTokenResponse(auth2) {
    switch (this.signInMethod) {
      case "password":
        return signInWithPassword$1(auth2, {
...
const authGoogle = () => signInWithPopup(auth, new GoogleAuthProvider());
...
const login = (onNavigate2) => {
  ...
  passwordLabel.innerHTML = "Contrase\xF1a:";
...
const register = (onNavigate2) => {
  ...
  SignInLabel.innerHTML = "\xBFYa tienes cuenta?";
  loginBtn.setAttribute("id", "loginBtn");
  ...
  loginBtn.addEventListener("click", () => onNavigate2("/login"));
  formRegister.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (error.code === "auth/weak-password") {
        alert("error en contrase\xF1a");
      }
      if (error.code === "auth/email-alre-in-use") {
        alert("El correo ya est\xE1 registrado");
      }
    }
  });
  BtnGoogle.addEventListener("click", async () => {
...
const welcome = (onNavigate2) => {
  ...
  <span style="color: black;">donde podr\xE1s compartir y publicar informaci\xF3n de tus mascotas</span>
  `;
  ...
  nextButton.addEventListener("click", () => onNavigate2("/timeline"));
...
class $h extends vh {
  _toFieldTransform(t2) {
    return new jn(t2.path, new $n());
  }
  isEqual(t2) {
    return t2 instanceof $h;
  }
}
class Bh extends vh {
  constructor(t2, e) {
...
const db = uh(app);
const getTimestamp = () => df();
const savePublic = (publicacion, likes, name2, email, time) => Xl(Ja(db, "publication"), {
  publicacion,
  likes,
  name: name2,
  email,
...
const postData = () => ol(Ja(db, "publication"), dl("time", "desc"));
const deletePost = (id2) => Zl(Za(db, "publication", id2));
const updatePost = (id2, newDocument) => Yl(Za(db, "publication", id2), newDocument);
const like = (docum, auth2) => {
  const docRef = Za(db, "publication", docum.id);
  return Yl(docRef, { likes: _f(auth2.currentUser.uid) });
};
const dislike = (docum, auth2) => {
  const docRef = Za(db, "publication", docum.id);
  return Yl(docRef, { likes: wf(auth2.currentUser.uid) });
};
const timeline = (onNavigate2) => {
  ...
  inputContainer.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      await savePublic(inputPost.value, [], name2, email, getTimestamp());
  ...
  homeIcon.addEventListener("click", () => onNavigate2("/"));
  profileIcon.addEventListener("click", () => onNavigate2("/welcome"));
  logOutIcon.addEventListener("click", async () => {
    await signOff();
    onNavigate2("/");
  });
  tf(postData(), (querySnapshot) => {
    ...
    querySnapshot.forEach((docum) => {
      ...
      dialogTitle.textContent = "Eliminar publicaci\xF3n";
      dialogMessage.textContent = "\xBFEst\xE1s seguro que deseas eliminar esta publicaci\xF3n?";
        ...
        editBtn.addEventListener("click", () => {
          ...
          saveBtn.addEventListener("click", () => {
            updatePost(docum.id, {
              ...
              console.log("Documento actualizado con \xE9xito");
