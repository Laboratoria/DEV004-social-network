...
var wa$1 = x$1("Opera"), y = x$1("Trident") || x$1("MSIE"), xa = x$1("Edge"), ya$1 = xa || y, za = x$1("Gecko") && !(-1 != sa$1().toLowerCase().indexOf("webkit") && !x$1("Edge")) && !(x$1("Trident") || x$1("MSIE")) && !x$1("Edge"), Aa$1 = -1 != sa$1().toLowerCase().indexOf("webkit") && !x$1("Edge");
function Ba() {
  var a = l.document;
...
const savedUser = (displayName, email, password, petName, petSpecie, uid) => Ul(Aa(db, "users", uid), {
  displayName,
  email,
  password,
  petName,
...
const colRef = Ta(db, "userpost");
const post = async (postText) => {
  await Ql(Ta(db, "userpost"), {
    text: postText,
...
const updatePost = (id2, newPost) => Kl(Aa(db, "userpost", id2), newPost);
const deleteDocData = async (id2) => {
  await Gl(Aa(db, "userpost", id2));
};
const like = async (id2, uid) => Kl(Aa(db, "userpost", id2), { likes: of(uid) });
const disLike = async (id2, uid) => Kl(Aa(db, "userpost", id2), { likes: uf(uid) });
const home = (onNavigate2) => {
  ...
  singInDiv.innerHTML += `<header>
    ...
    <h1>Inicio de sesi\xF3n</h1>
    <input type="email" id="email" placeholder="Ingrese su correo" /> 
    <input type="password" id="password" placeholder="Ingrese su contrase\xF1a"/>
    <button type="submit" id="signin-button">Iniciar Sesi\xF3n</button>
    <p>\xBFNo tienes cuenta?,</p><a id="here">Registrate aqu\xED</a>
    <img src="./img/google.png" id="google">`;
  root2.appendChild(singInDiv);
  ...
  submitButton.addEventListener("click", () => {
    ...
    signIn(signInEmail, signInPassword).then((usercredentials) => {
...
const register = (onNavigate2) => {
  ...
  <section class="register-container">
    <h1>T\xFA</h1>
      ...
      <input type="password" id="register-password" placeholder="Ingrese una contrase\xF1a"/>
      <input type="password" id="register-password2" placeholder="Repita su contrase\xF1a"/>
    <h1>Tu mascota</h1>
  ...
  document.querySelector("#create-account").addEventListener("click", () => {
    ...
    createUser(signUpEmail, signUpPassword).then((usercredentials) => {
      const user = usercredentials.user;
      if (displayName) {
...
function validateData() {
  ...
  if (signUpRepeatPssword === "") {
    alert("Ingrese la repeticion de la contrase\xF1a");
    return false;
  }
    ...
    <section class='timeline'>
      <section class='create-post-container'>
        <section class='create-post'>
          <h2>T\xFA</h2>
          <textarea id='status-description' placeholder=' \xBFQue hizo tu animal de compa\xF1\xEDa hoy?' maxlength='300'></textarea>
        </section>
      </section>
    </section>
    ...
    posts.forEach((feedPosts) => {
      ...
      if (feedPosts.userId === auth.currentUser.uid) {
        ...
        deleteButton.addEventListener("click", () => {
          const shouldDelete = window.confirm("\xBFEst\xE1s seguro de que deseas eliminar este post?");
          if (shouldDelete) {
        ...
        saveButton.addEventListener("click", () => {
          ...
          updatePost(refPostId, { text: newPostText.value }).then(() => {
            editSection.style.display = "none";
          });
        });
