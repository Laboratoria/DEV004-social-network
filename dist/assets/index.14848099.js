...
const savePublic = (publicacion, likes, name2, time) => Xl(Ja(db, "publication"), {
  publicacion,
  likes,
  name: name2,
  time
...
const timeline = (onNavigate2) => {
  ...
  inputContainer.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      await savePublic(inputPost.value, [], name2, getTimestamp());
  ...
  tf(postData(), (querySnapshot) => {
    ...
    querySnapshot.forEach((docum) => {
      ...
      likePawZero.addEventListener("click", () => {
        ...
        const likeImage = document.getElementById("likePawZero");
        if (!likes.includes(user)) {
          like(docum, auth);
          likeImage.src = "../img/likePawZero.png";
        } else {
          dislike(docum, auth);
          likeImage.src = "likePaw.jpg";
        }
      });
