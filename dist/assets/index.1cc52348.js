...
!function(t2, e = true) {
  ...
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(v, "3.10.1", t2), registerVersion(v, "3.10.1", "esm2017");
}();
/**
 * @license
 * Copyright 2017 Google LLC
...
async function getPost() {
  const postCol = sh(db, "posts");
  const postSnapshot = await tf(postCol);
...
async function editPost(id2, description) {
  console.log("id: ", id2);
...
async function deletePost(id2) {
  const docRef = rh(db, "posts", id2);
  of(docRef).then(() => {
    alert("Post Eliminado ", id2);
    location.reload();
  }).catch(() => {
    console.error("Error al eliminar post", id2);
  });
}
const entrarConGoogle = () => signInWithPopup(auth, provider).then((result) => {
...
const home = () => {
  ...
  spanButton.textContent = "Inicia sesi\xF3n con Google";
  divButton.append(spanButton);
  buttonGoogle.append(divButton);
...
const register = () => {
  ...
  nombre.textContent = "Nombre de tu mascota";
...
const wall = () => {
  ...
  divForm.innerHTML = `<div id="regresar">
                      ...
                      <input class="input" id="raza" type="text" placeholder="Raza de tu mascota aqu\xED">
                      <input class="input" id="urlphoto" type="text" placeholder="Ingresa la Url de tu foto aqu\xED">
                      <input class="input" id="description" type="text" placeholder="Ingresa tu post aqu\xED">
                      <button id="crear-btn">Enviar</button></div>`;
  const button = divForm.querySelector("#crear-btn");
  button.addEventListener("click", (e) => {
    const name2 = divForm.querySelector("#name").value;
    ...
    if (data.length > 1) {
      data.forEach((post) => {
        ...
        postContainer.innerHTML = `<div><h4 id="postTitle">Mi nombre es:${post.name}</h4></div>
          ...
          <div><button postId=${post.id} id="${post.id}b" class="b-edit"  style="display: none;" id="Confirmar">Confirmar Edici\xF3n</button></div>
        ...
        document.querySelectorAll("button").forEach((boton) => {
          boton.addEventListener("click", (event) => {
            const tag = event.target.getAttribute("postId");
